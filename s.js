//its whole scraping script by add json of that total village.
...
let counter = 0;
const villageList = data.map(item => item.code);
...
const dist = "Enter Distic Name";
const tal = "Enter Taluka Name";
...
async function scrapeVillageData() {
    ...
    try {
        ...
        await page.select("#level_2", "10");


        //Taluka Code
        await page.waitForSelector("#level_3");
        await page.select("#level_3", "04");
        ...
        // Save all village data as a single JSON file
        const dirPath = `./${dist}/${tal}`;
        fs.mkdirSync(dirPath, {recursive: true});

        const jsonFilePath = `${dirPath}/all_village_data_testing_${tal}.json`;
